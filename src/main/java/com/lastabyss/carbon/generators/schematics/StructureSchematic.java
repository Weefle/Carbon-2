...
public class StructureSchematic {
		...
		if ((size.getX() >= 1) && (size.getY() >= 1) && (size.getZ() >= 1)) {
			...
			for (MutableBlockPosition blockPos : BlockPositionUtil.allInCubeM(minCorner, maxCorner)) {
				...
				if (te != null) {
					...
					tenbtdata.remove("y");
					tenbtdata.remove("z");
					blocksWithTileEntity.add(new StructureSchematic.BlockData(relativePos, blockData, tenbtdata));
				} else if (!blockData.getBlock().o() && !blockData.getBlock().d()) {
					notCubeBlocks.add(new StructureSchematic.BlockData(relativePos, blockData, null));
				} else {
					normalBlocks.add(new StructureSchematic.BlockData(relativePos, blockData, null));
				}
			}
	...
	private void setEntities(World world, BlockPosition minCorner, BlockPosition maxCorner) {
		...
		for (Iterator<Entity> iterator = entities.iterator(); iterator.hasNext(); this.entities.add(new StructureSchematic.EntityData(vec, blockPos, nbtdata))) {
	...
	public Map<BlockPosition, String> a(BlockPosition var1, class_asu var2) {
		...
		while (true) {
			StructureSchematic.BlockData var6;
			BlockPosition var7;
			do {
				if (!var5.hasNext()) {
					return var3;
	...
	public BlockPosition a(class_asu var1, BlockPosition var2, StructureSchematic var3, class_asu var4, BlockPosition var5) {
	...
	public BlockPosition a(EnumRotation var1) {
		switch (StructureSchematic.SyntheticClass_1.a[var1.ordinal()]) {
			case 1:
			case 2:
	...
	private BlockPosition a(BlockPosition var1, class_a_in_class_agj var2, EnumRotation var3) {
		...
		int var6 = var1.getZ();
		boolean var7 = true;
		switch (StructureSchematic.SyntheticClass_1.b[var2.ordinal()]) {
			case 1:
				var6 = -var6;
				break;
			case 2:
		...
		switch (StructureSchematic.SyntheticClass_1.a[var3.ordinal()]) {
			case 1:
	...
	public void save(NBTTagCompound compound) {
		...
		for (Iterator<BlockData> it = this.blocks.iterator(); it.hasNext(); blocksTag.add(blockTag)) {
			StructureSchematic.BlockData data = it.next();
			blockTag = new NBTTagCompound();
			blockTag.set("pos", this.a(data.position.getX(), data.position.getY(), data.position.getZ()));
		...
		for (Iterator<EntityData> it = this.entities.iterator(); it.hasNext(); entitiesTag.add(entityTag)) {
			StructureSchematic.EntityData data = it.next();
			entityTag = new NBTTagCompound();
			entityTag.set("pos", this.a(data.vec.a, data.vec.b, data.vec.c));
			entityTag.set("blockPos", this.a(data.position.getX(), data.position.getY(), data.position.getZ()));
			if (data.tag != null) {
		...
		compound.set("size", this.a(this.size.getX(), this.size.getY(), this.size.getZ()));
	...
	public void load(NBTTagCompound compound) {
		...
		for (int i = 0; i < blocksTag.size(); ++i) {
			...
			this.blocks.add(new StructureSchematic.BlockData(pos, blockData, nbtdata));
		}
		...
		for (int i = 0; i < entitiesTag.size(); ++i) {
			...
			if (entityTag.hasKey("nbt")) {
				...
				this.entities.add(new StructureSchematic.EntityData(vec, blockPos, nbtdata));
			}
		}
